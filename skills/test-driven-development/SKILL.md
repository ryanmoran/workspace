...
```bash
go test -count=1 -run TestRetryOperation ./resilience
```
...
```bash
$ go test -count=1 -run TestRetryOperation .
--- FAIL: TestRejectsEmptyEmail (0.00s)
    form_test.go:31:
            Error Trace:    /home/root/project/form_test.go:31
                                                    /home/root/project/form_test.go:69
            Error:          Error message should contain "email required"
            Test:           TestRejectsEmail
```

**GREEN**

```typescript
func SubmitForm(email string) error {
